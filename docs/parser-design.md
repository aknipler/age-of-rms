...
3. **Words and numbers can alias structural tokens** (`MILL` can close a block). Out of scope for structural emulation in v1: such maps misparse into diagnostics + RawNodes, never crashes or dropped text. `ParseOptions.aliasTable` is the upgrade hook — import the Equivalencies data later as `reference/data/token-aliases.json` (fetch failed in-session: JS-rendered; import manually, spot-verify against DE — ⚠ verify #7).
   ...
   **`aliasTable`'s declared type cannot express the case that now motivates it.** It is `ReadonlyMap<string, TokenKind>`, which covers a word aliasing a *structural* token — the `MILL` shape above. The live corpus case is a word aliasing a **command**: `24hr_Petra.rms:6` writes `#const L 32` and then uses `L { … }` 384 times, `24hr_Holler.rms` 197 more, and those 581 sites were **68% of RMS0200's entire corpus output** *(52 `.rms`)*. So the tier-2 fix is not a drop-in for this type and the option is not the upgrade hook it reads as.

4. **A word aliasing a COMMAND is resolved, and the id lives on `CommandDef`. AMENDED 2026-08-13 (BUG-005 piece 2) — this supersedes item 3's `token-aliases.json` for the command case, and CREATION_PLAN A.2 carries the same amendment.** The optional `tokenId` field on a `CommandDef` in `language.json` records the engine's own id for that command name, set only where a run has measured it and only alongside a `notes` entry naming the run. `buildLanguageIndex` builds `commandsByTokenId` from it; `parseNamedOrRun` consults the script's own `#const` symbol table for any word that resolves as neither command nor attribute, and a plain decimal value matching a known `tokenId` supplies the `def`. `CommandNode.name` keeps the author's token, so nothing re-prints code. Corpus RMS0200 **858 → 277**, with all 21 true positives unchanged.

   **Why not `token-aliases.json`, which item 3 and A.2 both prescribed.** The id is an attribute of the command, not a relationship between two entities: `game-constants.json` already stores exactly this integer as `constId` **on the constant row**, and `validate.ts` reads it that way for RMS0111's id 69. Storing the command half of one flat namespace in a join table would make two halves of the same fact look like two kinds of data, and it costs a file, a schema, a loader and a join to hold **one row**. `CommandDef` also already carries `sectionLocked`, an optional engine-*measured* field with the same "cite the run" discipline, so the shape has a precedent on this exact type. The equivalencies sheet remains CREATION_PLAN A.2's general case; if it lands, it is a *source* for `tokenId` and for whatever the structural aliases of item 3 then need, not a reason to move the field.

   **`#const` may alias a NAME as well as an id, and that is the same ruling.** `24hr_Battle Lines 1.0.rms:93` writes `#const restricted_terrain_distance max_distance_to_other_zones`. Sec.6's stop set halted `#const`'s value consumption on the known name, so the author's correct line drew a too-few-arguments warning plus an orphaned-attribute diagnostic. The new `ArgumentDef.acceptsKnownName` flag drops the known-name half of the stop set for that one slot; the structural half never yields, or a valueless `#const` would swallow the section header after it. Deliberately a per-argument flag rather than a rule keyed on `type: "otherConstant"` — 15 other slots share that type and on `effect_amount.attribute` or `assign_to.target` a known name really is a mangled line. Corpus RMS0201 **22 → 20**.

   **What is NOT built, and is not an oversight.** A `#const` whose value is another *command name* (`#const MY_LAND create_land`) is not resolved: zero corpus sites, and a rule with no measured population is the speculative generality this file keeps cataloguing. Nor is the value expression evaluated — `SymbolInfo` carries the value's token, not a number, and the literal case is the whole of the corpus. Both fall back to today's RMS0200, which is the conservative direction.
...
interface ParseOptions {
  ...
  aliasTable?: ReadonlyMap<string, TokenKind>;     // default empty (Sec.2.1) — STRUCTURAL aliases only.
                                                   // Command aliasing does NOT go here: it is resolved
                                                   // from CommandDef.tokenId + the script's own #const
                                                   // symbol table (Sec.2.1 item 4), needs no option, and
                                                   // this type could never have expressed it.
